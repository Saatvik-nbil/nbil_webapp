...
Motion is layered and intentional, and several libraries coexist by role:
...
- **Webpack filesystem cache is disabled** in `next.config.ts` (`config.cache = false`) because this machine's disk fills and the pack cache overflows with ENOSPC. Rebuilds are slower but won't crash, so don't "fix" this by re-enabling the cache.
