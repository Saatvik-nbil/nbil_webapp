...
/* -------------------------------------------------------------------------- */
/*  Brand-adapted, reusable liquid glass for the NBIL site                     */
/*  (refined per /emil-design-eng + /ui-ux-pro-max: softer distortion so it    */
/*   doesn't warp content into slop, light/dark-aware tint with readable       */
/*   contrast, spring press feedback, reduced-motion safe.)                    */
/* -------------------------------------------------------------------------- */

// A gentler displacement filter for content-bearing UI surfaces. The demo's
// scale="200" is great for a photo backdrop but shreds text behind a panel.
...
type LiquidGlassProps = {
  children: React.ReactNode;
  className?: string;
  /** "light" frosts toward white (use on dark/photographic backdrops);
   *  "dark" frosts toward navy (use on bright backdrops). */
  tint?: "light" | "dark";
  /** Apply the refractive displacement. Turn OFF for text-dense surfaces
   *  like the nav bar, where warping would read as a glitch. */
  distort?: boolean;
  /** Adds a spring scale on press for tactile feedback. */
  interactive?: boolean;
  style?: React.CSSProperties;
};
...
export const LiquidGlass: React.FC<LiquidGlassProps> = ({
  ...
  return (
    ...
    >
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 rounded-[inherit]" style={{ boxShadow: innerHighlight }} />
      {children}
    </div>
  );
};
