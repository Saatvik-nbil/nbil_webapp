...
"Anyone" means anyone who knows the URL can POST, but it does **not** make the
...
Use *Manage deployments* rather than *New deployment*, since it keeps the same
...
needs to change, since both selects and the server rule read from that array.
