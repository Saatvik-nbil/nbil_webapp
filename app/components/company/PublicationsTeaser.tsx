...
/**
 * Landing-page proof band: the first-page renders of the three papers chosen
 * for international reach, linking through to /publications where every
 * record, abstract and DOI lives. The picks are set in lib/publications.
 */
              ...
              >
                <div className="overflow-hidden rounded-xl border border-[var(--color-hairline)] bg-white">
                  {pub.thumb ? (
                    ...
                    /* No PDF on file to render a first page from, so the card
                       sets the journal instead of showing a broken image. */
                    <div className="flex aspect-[3/4] w-full flex-col justify-between bg-[var(--color-surface-raised)] p-4">
                      <span className="text-[12.5px] font-semibold text-[var(--color-brand-strong)]">
                        {pub.journal}
                      </span>
                      <span className="font-display text-[15px] font-semibold leading-[1.3] tracking-[-0.015em] text-[var(--color-ink)] line-clamp-5">
                        {pub.title}
                      </span>
                    </div>
                  )}
